...
A333DR_gpws_sys_state = find_dataref("laminar/A333/gpws/system_state") -- 0 = off, 1 = powered/no valid GPS, 2 = ready
...
local function receiving_nav(index, n_type)
    ...
    local enum = nav_type[index]
    local t = {}
    local r = {}

    for i=14,-1,-1 do
        t[#t+1] = m.floor(enum / 2^i)
        enum = enum % 2^i
    end
...
local function set_gpws_message()
...
local function get_message_by_id(id)
...
local function play_message(id, val)
...
local function predictive_windshear()
    ...
    local windshear_ws = A333DR_windshear_ws

    if (DR_windshear_ws ~= last_windshear) then
        last_windshear = DR_windshear_ws
        windshear_ws = DR_windshear_ws
        if windshear_ws == 2 then play_message('monitor_radar_display', true) end
        if windshear_ws == 3 then play_message('windshear_ahead', true) end
        if windshear_ws == 4 then play_message('go_around_windshear_ahead', true) end
        if windshear_ws == 5 then play_message('windshear', true) end
        windshear_ws = 0
    end
    A333DR_windshear_ws = windshear_ws
end

local function airbus_overrides()
...
local function mode_6_bank_angle()
    local roll_angle = m.abs(DR_roll_indicator)
    local rad_alt = DR_rad_alt

    if DR_autopilot_on == 0 then

        if rad_alt > 5 and rad_alt < 30 then
        ...
        elseif rad_alt >= 30 and rad_alt < 150 then
            local min_alt = 4 * roll_angle - 10;
            if roll_angle >= 10 and roll_angle < 40 and rad_alt < min_alt then
                if not is_bank_angle_played then
                    play_message('bank_angle', true)
                    is_bank_angle_played = true
                end
            end
        elseif rad_alt >= 150 and rad_alt < 2450 then
            local min_alt = 153.333 * roll_angle - 5983.33;
            if roll_angle >= 40 and roll_angle < 55 and rad_alt < min_alt then
                if not is_bank_angle_played then
                    play_message('bank_angle', true)
                    is_bank_angle_played = true
                end
            end
        elseif rad_alt >= 2450 then
    ...
    elseif DR_autopilot_on == 1 then
        ...
        elseif rad_alt >= 30 and rad_alt < 122 then
            local min_alt = 4 * roll_angle - 10;
            if roll_angle >= 10 and roll_angle < 33 and rad_alt < min_alt then
                if not is_bank_angle_played then
                    play_message('bank_angle', true)
                    is_bank_angle_played = true
                end
            end
        elseif rad_alt >= 122 then
...
local function mode_1_sinkrate()

    local rad_alt = DR_rad_alt
    local vario = DR_vario
    local min_alt = 0.64473 * m.abs(vario) - 644.73


    if rad_alt < 2450 and vario < 0 and vario > -4800 and rad_alt < min_alt then
...
local function mode_4_too_low()
    ...
    local gear_ratio = DR_gear_ratio
    local on_ground = DR_on_ground
    local airspeed = DR_airspeed
    local rad_alt = DR_rad_alt
    local vario = DR_vario

    -- Mode 4a
    if gear_ratio[0] < 0.01
        and gear_ratio[1] < 0.01
        and gear_ratio[2] < 0.01
        and on_ground == 0
    then

        if airspeed < 190 and rad_alt < 500 and vario < -300 then
        ...
        local min_alt = 2.3809 * airspeed + 47.6190
        if airspeed >= 190 and rad_alt < 1000 and rad_alt < min_alt and vario < -300 then
    ...
    then
        ...
        if airspeed < 150 and (rad_alt < 245 and rad_alt > 30) then
        ...
        local min_alt = 7.55 * airspeed - 887.5
        if airspeed >= 150 and rad_alt < 1000 and rad_alt < min_alt and vario < -300 then
...
local function mode_5_below_glideslope()
    ...
    if A333DR_gpws_sys_state == 2
        and A333DR_gpws_GS_tog_pos == 1
        and receiving_nav(0, 'ils')
        and DR_nav_vert_signal[0] == 1
        and (flight_phase >=6 and flight_phase <= 7)    -- Prevents spurious warning on takeoff
    then
        mode5_isArmed = true
    end
    ...
    if mode5_isArmed
        and rad_alt >= 30.0
        and rad_alt <= 1000.0
        and gear_ratio[0] > 0.99
        and gear_ratio[1] > 0.99
        and gear_ratio[2] > 0.99
    then

        local dots_below_gs = m.abs(rescale(-2.5, -4.0, 0.0, 0.0, DR_vdef_dots_capt))
        if isInEnvelope(dots_below_gs, rad_alt, mode5_hard_warning_envelope) then
            mode5 = 2
        elseif isInEnvelope(dots_below_gs, rad_alt, mode5_soft_warning_envelope) then
            mode5 = 1
        end
    end
...
local function save_current_time()
    last_time = DR_running_time;
end

-- Ensure reset status of GPWS on replay
local function update_replay_status()
