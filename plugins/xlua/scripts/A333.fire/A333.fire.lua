...
simDR_fire_ext_eng					= find_dataref("sim/cockpit2/engine/actuators/fire_extinguisher_on")
...
simDR_apu_bleed_press_fail			= find_dataref("sim/operation/failures/rel_APU_press")
...
--*************************************************************************************--
local function fix_all_systems_beforeCMDhandler(phase, duration) end
local function fix_all_systems_afterCMDhandler(phase, duration)
    if phase == 0 then
...
----- ANIMATION UTILITY -----------------------------------------------------------------
local function animate(current_value, target, speed)
...
local function A333_fire_handle_animation()

	A333_eng1_fire_handle_pos = animate(A333_eng1_fire_handle_pos, engine1_handle_target, 8)
	A333_eng2_fire_handle_pos = animate(A333_eng2_fire_handle_pos, engine2_handle_target, 8)
	A333_apu_fire_handle_pos = animate(A333_apu_fire_handle_pos, apu_handle_target, 8)

end



local function A333_cargo_fire_test_seq()

	local cargo_fire_test = A333_cargo_fire_test

	cargo_test_trigger = (cargo_fire_test_timer >= 3 and 1) or 0
	cargo_fire_test_seq = cargo_test_trigger == 1 and (cargo_fire_test_seq + lcl_SIM_PERIOD) or 0

	if cargo_fire_test_seq > 10 then cargo_test_trigger = 0 end

	cargo_fire_test = (cargo_fire_test_seq > 0 and 1) or 0

	A333_cargo_fire_test = cargo_fire_test
...
local function A333_extinguisher_agent_discharge()
	...
	local eng1_agent1_psi = A333_eng1_agent1_psi
	local eng1_agent2_psi = A333_eng1_agent2_psi
	local eng2_agent1_psi = A333_eng2_agent1_psi
	local eng2_agent2_psi = A333_eng2_agent2_psi
	local apu_agent_psi = A333_apu_agent_psi
	local eng_fire_ext = simDR_fire_ext_eng

	-- AGENT DISCHARGES
	if engine1_agent1_discharge == 1 then
		eng1_agent1_psi = m.max(0, eng1_agent1_psi - (40.0 * lcl_SIM_PERIOD))
	end

	if engine1_agent2_discharge == 1 then
		eng1_agent2_psi = m.max(0, eng1_agent2_psi - (40.0 * lcl_SIM_PERIOD))
	end

	if engine2_agent1_discharge == 1 then
		eng2_agent1_psi = m.max(0, eng2_agent1_psi - (40.0 * lcl_SIM_PERIOD))
	end

	if engine2_agent2_discharge == 1 then
		eng2_agent2_psi = m.max(0, eng2_agent2_psi - (40.0 * lcl_SIM_PERIOD))
	end

	if apu_agent_discharge == 1 then
		apu_agent_psi = m.max(0, apu_agent_psi - (40.0 * lcl_SIM_PERIOD))
	end
	...
	if engine1_agent1_discharge == 1 then
		if eng1_agent1_psi > 0 then
			extinguisher1a = 1
		elseif eng1_agent1_psi == 0 then
			extinguisher1a = 0
	...
	if engine1_agent2_discharge == 1 then
		if eng1_agent2_psi > 0 then
			extinguisher1b = 1
		elseif eng1_agent2_psi == 0 then
			extinguisher1b = 0
	...
	if engine2_agent1_discharge == 1 then
		if eng2_agent1_psi > 0 then
			extinguisher2a = 1
		elseif eng2_agent1_psi == 0 then
			extinguisher2a = 0
	...
	if engine2_agent2_discharge == 1 then
		if eng2_agent2_psi > 0 then
			extinguisher2b = 1
		elseif eng2_agent2_psi == 0 then
			extinguisher2b = 0
	...
	eng_fire_ext[0] = ((extinguisher1a == 1 or extinguisher1b == 1) and 1) or 0
	eng_fire_ext[1] = ((extinguisher2a == 1 or extinguisher2b == 1) and 1) or 0



	extinguisher_apu_trigger = ((apu_agent_discharge == 1 and (apu_agent_psi > 25 and apu_agent_psi < 30)) and 1) or 0
	apu_agent_discharge = ((apu_agent_discharge == 1 and apu_agent_psi == 0) and 0) or apu_agent_discharge

	if extinguisher_apu_trigger	== 1 then
		simDR_apu_fire = 0
	end

	A333_eng1_agent1_psi = eng1_agent1_psi
	A333_eng1_agent2_psi = eng1_agent2_psi
	A333_eng2_agent1_psi = eng2_agent1_psi
	A333_eng2_agent2_psi = eng2_agent2_psi
	A333_apu_agent_psi = apu_agent_psi
...
local function A333_kill_systems()
...
local function A333_set_fire_CD()
...
local function A333_set_fire_ER()
...
local function A333_set_fire_all_modes()
...
local function A333_fire_monitor_AI()
...
----- FLIGHT START ---------------------------------------------------------------------
local function A333_flight_start_fire()
