...
local epr_takeoff = A333_epr_takeoff
local epr_limit_to = A333DR_epr_limit_to
local epr_limit_flex = A333DR_epr_limit_flex
local epr_limit_mc = A333DR_epr_limit_mc
local epr_limit_ga = A333DR_epr_limit_ga
local epr_max_cont = A333_epr_max_cont
local epr_go_around = A333_epr_go_around
...
local function rescale2(in1, out1, in2, out2, x)
...
local function A333_engine_aircon_status()

	local engines_running = simDR_engines_running

	airConOn = {0.0, 0.0}

	-- ENGINE 1
	if engines_running[0] == 1
...
local function A333_epr_TO()

	local altitude_msl = simDR_altitude_msl

	local eprAdjAirConOn	= {-0.006, -0.010}
	local oatAdjAInacelleOn	= {0.9, 1.20}
	local oatAdjAIwingOn	= {0.8, 1.30}
	local oatAdjAC			= rescale(7500.0, eprAdjAirConOn[1], 8500.0, eprAdjAirConOn[2], altitude_msl)
	local oatAdjAInacelle	= rescale(7500.0, oatAdjAInacelleOn[1], 8500.0, oatAdjAInacelleOn[2], altitude_msl)
	local oatAdjAIwing		= rescale(7500.0, oatAdjAIwingOn[1], 8500.0, oatAdjAIwingOn[2], altitude_msl)
	...
	if altitude_msl <= -2000.0 then
		altLOindex = -2000.0
	else
		for index = -2000.0, 15000.0, 1000.0 do
			if index <= altitude_msl then
				altLOindex = index
			end
		end
	end
	...
	for i = 1, 2 do
		...
		altDeltaRatio		= rescale2(altLOindex, 0.0, altHIindex, 1.0, altitude_msl)

		-- CALC LO/HI EPR VALUES
		eprAltLO[i]			= rescale(0.0, epr_takeoff[oatLOindex[i]][altLOindex], 1.0, epr_takeoff[oatHIindex[i]][altLOindex], oatDeltaRatio[i])
		eprAltHI[i]			= rescale(0.0, epr_takeoff[oatLOindex[i]][altHIindex], 1.0, epr_takeoff[oatHIindex[i]][altHIindex], oatDeltaRatio[i])
	...
	epr_limit_to[0] = eprTO[1]
	epr_limit_to[1] = eprTO[2]
	epr_limit_to[2]	= (eprTO[1] + eprTO[2]) * 0.50
...
local function A333_epr_FLEX()
	...
	-- SET THE EPR DATAREF VALUES
	epr_limit_flex[0] = eprFLEX[1]
	epr_limit_flex[1] = eprFLEX[2]
	epr_limit_flex[2] = (eprFLEX[1] + eprFLEX[2]) * 0.50
...
local function A333_epr_MC()
	...
	for i = 1, 2 do
		...
		-- CALC LO/HI EPR VALUES
		eprAltLO[i]			= rescale(0.0, epr_max_cont[tatLOindex[i]][altLOindex], 1.0, epr_max_cont[tatHIindex[i]][altLOindex], tatDeltaRatio[i])
		eprAltHI[i]			= rescale(0.0, epr_max_cont[tatLOindex[i]][altHIindex], 1.0, epr_max_cont[tatHIindex[i]][altHIindex], tatDeltaRatio[i])
	...
	-- SET THE EPR DATAREF VALUES
	epr_limit_mc[0] = eprMC[1]
	epr_limit_mc[1] = eprMC[2]
	epr_limit_mc[2]	= (eprMC[1] + eprMC[2]) * 0.50
...
local function A333_epr_GA()
	...
	local tatAdjAC			= rescale(7500.0, eprAdjAirConOn[1], 8500.0, eprAdjAirConOn[2], altitude_msl)
	local tatAdjAInacelle	= rescale(7500.0, tatAdjAInacelleOn[1], 8500.0, tatAdjAInacelleOn[2], altitude_msl)
	local tatAdjAIwing		= rescale(7500.0, tatAdjAIwingOn[1], 8500.0, tatAdjAIwingOn[2], altitude_msl)
	local tatAdjusted		= {0.0, 0.0}
	local tatLOindex		= {0.0, 0.0}
	local tatHIindex		= {0.0, 0.0}
	local altLOindex		= 0.0
	...
	for i = 1, 2 do
		...
		-- CALC LO/HI EPR VALUES
		eprAltLO[i]			= rescale(0.0, epr_go_around[tatLOindex[i]][altLOindex], 1.0, epr_go_around[tatHIindex[i]][altLOindex], tatDeltaRatio[i])
		eprAltHI[i]			= rescale(0.0, epr_go_around[tatLOindex[i]][altHIindex], 1.0, epr_go_around[tatHIindex[i]][altHIindex], tatDeltaRatio[i])
	...
	-- SET THE EPR DATAREF VALUES
	epr_limit_ga[0] 	= eprGA[1]
	epr_limit_ga[1] 	= eprGA[2]
	epr_limit_ga[2]	= (eprGA[1] + eprGA[2]) * 0.50
...
local function A333_ALL_engine02()
