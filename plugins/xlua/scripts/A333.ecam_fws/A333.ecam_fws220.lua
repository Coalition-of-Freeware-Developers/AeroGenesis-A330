...
local sts_pulseL01 = newLeadingEdgePulse("sts_pulseL01")
local sts_pulseL02 = newLeadingEdgePulse("sts_pulseL02")
...
local function A333_fws_ecp_EMER_CANC()

	emcx_pulseR01:update(WEC_2)
	emcx_pulseR02:update(WEMERC)
	emcx_pulseF03:update(WEC_2)
	emcx_pulseF04:update(WEMERC)
	emcx_mTrigR01:update(emcx_pulseR01.OUT or emcx_pulseR02.OUT)
	emcx_mTrigR02:update(emcx_pulseF03.OUT or emcx_pulseF04.OUT)
...
local function A333_fws_ecp_CLR()

	clr_pulseR01:update(WCLR1_2)
	clr_pulseR02:update(WCLR2_2)
	clr_pulseR03:update(WCLR)
	clr_mTrigR01:update(clr_pulseR01.OUT or clr_pulseR02.OUT or clr_pulseR03.OUT)
...
local function A333_fws_ecp_STS()

	sts_pulseL01:update(WSTS_2)
	sts_pulseL02:update(WSTS)
	sts_pulseF03:update(WSTS_2)
	sts_pulseF04:update(WSTS)
	sts_mTrigR01:update(sts_pulseL01.OUT or sts_pulseL02.OUT)
	sts_mTrigR02:update(sts_pulseF03.OUT or sts_pulseF04.OUT)

	ZSTSUP = sts_mTrigR01.OUT
	ZSTSDN = sts_mTrigR02.OUT
    ZSTSLVL = WSTS_2 or WSTS

end



local function A333_fws_ecp_RCL()

	rcl_pulsePush01:update(WRCL_2)
	rcl_pulsePush02:update(WRCL)
	rcl_pulseRelease03:update(WRCL_2)
	rcl_pulseRelease04:update(WRCL)
	local a = rcl_pulsePush01.OUT or rcl_pulsePush02.OUT
	local b = rcl_pulseRelease03.OUT or rcl_pulseRelease04.OUT
	rcl_mTrigPush01:update(a)
	rcl_mTrigRelease02:update(b)
	local c = WRCL_2_INV or WRCL_INV
	local d = rcl_mTrigRelease02.OUT and (not c)

	ZRCLUP = rcl_mTrigPush01.OUT	-- BUTTON PUSH
	ZRCLDN = d	                    -- BUTTON RELEASE

end



local function A333_fws_ecp_ENG()
...
local function A333_fws_ecp_BLEED()
...
local function A333_fws_ecp_PRESS()
...
local function A333_fws_ecp_EL_AC()
...
local function A333_fws_ecp_EL_DC()
...
local function A333_fws_ecp_HYD()
...
local function A333_fws_ecp_CB()
...
local function A333_fws_ecp_APU()
...
local function A333_fws_ecp_COND()
...
local function A333_fws_ecp_DOOR()
...
local function A333_fws_ecp_WHEEL()
...
local function A333_fws_ecp_F_CTL()
...
local function A333_fws_ecp_FUEL()
...
local function A333_fws_ecp_ALL()

	all_pulseR01:update(WALL_2)
	all_pulseR02:update(WALL)
	all_mTrigR01:update(all_pulseR01.OUT or all_pulseR02.OUT)
