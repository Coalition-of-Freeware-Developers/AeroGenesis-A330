...
function newLeadingEdgePulse(name)
	...
	function pulse:update(input)
		self.IN 	= input
		self.OUT 	= ((self.IN) ~= (self.lastIN)) and (self.IN)
		self.lastIN = self.IN
	end
...
function newFallingEdgePulse(name)
	...
	function pulse:update(input)
		self.IN 	= input
		self.OUT 	= ((self.IN) ~= (self.lastIN)) and (not self.IN)
		self.lastIN	= self.IN
	end
...
function newLeadingEdgeTrigger(name, duration)
	...
	function trigger:update(input)
		self.IN = input
		self.START = ((self.IN) ~= (self.lastIN)) and (self.IN)
		if self.START then
			self.OUT = true
			self:startTimer()
		end
...
function newFallingEdgeTrigger(name, duration)
	...
	function trigger:update(input)
		self.IN = input
		self.START = ((self.IN) ~= (self.lastIN)) and (not self.IN)
		if self.START then
			self.OUT = true
			self:startTimer()
		end
...
function newFallingEdgeDelayedConfirmation(name, duration)
	...
	function conf:update(input)
		self.IN = input
		self.START = ((self.IN) ~= (self.lastIN)) and (not self.IN) and (self.OUT)
		if self.START then
			self:startTimer()
		end
		if self.IN
...
function newSRlatchSetPriority(name)
	...
	function srS:update(s, r)
		...
		self.gate1.E1	= not (self.S)
		self.gate1.E2	= self.R
		self.gate1.OUT	= (self.gate1.E1) and (self.gate1.E2)

		self.gate2.E1	= self.S
		self.gate2.E2	= self.gate3.OUT
		self.gate2.OUT	= not((self.gate2.E1) or (self.gate2.E2))

		self.gate3.E1	= self.gate2.OUT
		self.gate3.E2	= self.gate1.OUT
		self.gate3.OUT	= not((self.gate3.E1) or (self.gate3.E2))
