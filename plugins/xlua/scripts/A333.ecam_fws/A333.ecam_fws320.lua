...
--*************************************************************************************--
local function A333_fws_ewd_monitor()
	...
	for _, msg in pairs(A333_ewd_msg) do
		...
		elseif msg.Monitor.video.IN == 1 then
			...
			-- OUTPUT ACTIVATION
			local msg_inhib_phase = msg.Inhibit[A333DR_flight_phase]
			if msg_inhib_phase == 0													-- NOT FLIGHT PHASE INHIBITED
				or (msg_inhib_phase == 1 and ZFPION)								-- INHIBITED WITH INHIBITION OVERRIDE
			then
				output = 1
			end
