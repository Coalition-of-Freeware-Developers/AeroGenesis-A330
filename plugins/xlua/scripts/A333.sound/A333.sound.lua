...
local function A333_set_radio_volumes()

	local r_vol_pilot = radio_vol_pilot
	local r_vol_copilot = radio_vol_copilot

	-- I hate this.
	r_vol_pilot[0] = vol_pilot_0
	r_vol_pilot[1] = vol_pilot_1
	r_vol_pilot[2] = vol_pilot_2
	r_vol_pilot[3] = vol_pilot_3
	r_vol_pilot[4] = vol_pilot_4
	r_vol_pilot[5] = vol_pilot_5
	r_vol_pilot[6] = vol_pilot_6
	r_vol_pilot[7] = vol_pilot_7
	r_vol_pilot[8] = vol_pilot_8
	r_vol_pilot[9] = vol_pilot_9

	r_vol_copilot[0] = vol_copilot_0
	r_vol_copilot[1] = vol_copilot_1
	r_vol_copilot[2] = vol_copilot_2
	r_vol_copilot[3] = vol_copilot_3
	r_vol_copilot[4] = vol_copilot_4
	r_vol_copilot[5] = vol_copilot_5
	r_vol_copilot[6] = vol_copilot_6
	r_vol_copilot[7] = vol_copilot_7
	r_vol_copilot[8] = vol_copilot_8
	r_vol_copilot[9] = vol_copilot_9
...
local function A333_set_thrust_delta()

	local thr_delta = thrust_delta

	if (burning_fuel_1 > 0) then
		thr_delta[0] = throttle_lever_1 - ( (n1_1 - 20) * (1 / 83) ) -- 105-22 (n1 throttle range)
	else
		thr_delta[0] = 0
	end
	if (burning_fuel_2 > 0) then
		thr_delta[1] = throttle_lever_2 - ( (n1_2 - 20) * (1 / 83) )
	else
		thr_delta[1] = 0
	end
end
...
local function A333_set_n2_array()

	local eng_n2 = engine_n2
...
local function A333_set_packs_array()

	local pack_act = pack_actuators
	local pack_flw = pack_flow

	pack_act[0] = pack_1
	pack_act[1] = pack_2
	pack_flw[0] = pack_flow_1
	pack_flw[1] = pack_flow_2
...
local function A333_set_shades_array()

	local cp_shades = cockpit_shades
...
local function A333_set_energized_status()
	is_energized = ((bus_volts_1 > 20 or bus_volts_2 > 20) and 1) or 0
end
